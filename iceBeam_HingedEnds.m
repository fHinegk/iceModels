function [y,M,sigma] = iceBeam_HingedEnds(x,L, H,lambda,E,I,k, dh)
% '''
...
epsilon = L*dh*lambda*k * (cos(lambda*L) + cosh(lambda*L)) / ...
    (lambda*L*cos(lambda*L) + cosh(lambda*L)*lambda*L - sin(lambda*L) - sinh(lambda*L));
% deflection of the ice beam
y = epsilon/k * ...
    (1 - (cosh(lambda*x).*cos(lambda*xp) + cosh(lambda*xp).*cos(lambda*x)) / ...
    (cosh(lambda*L)+cos(lambda*L)));

% second derivative of the beam deflection
d2y = -epsilon*2*lambda^2*...
    (sin(lambda*x).*sinh(lambda*xp) + sinh(lambda*x).*sin(lambda*xp)) / ...
    (k*(cos(lambda*L) + cosh(lambda*L)));
